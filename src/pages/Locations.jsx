...
export default function Locations({ locations, setLocations }) {
  ...
  const handleSave = () => {
    if (!form.name || !form.capacity || !form.owner || !form.location || !form.status) {
      toast("All fields are required.", { style: { background: "#111", color: "#E5E5E5" } })
      return
    }

    if (editingLocation) {
      setLocations(locations.map((l) => (l.id === editingLocation.id ? { ...l, ...form } : l)))
      toast("Location updated successfully!", { style: { background: "#111", color: "#E5E5E5" } })
    } else {
      setLocations([...locations, { id: Date.now(), ...form }])
      toast("Location added successfully!", { style: { background: "#111", color: "#E5E5E5" } })
    }
  ...
  const handleDelete = (id) => {
    ...
    toast("Location deleted successfully.", { style: { background: "#111", color: "#E5E5E5" } })
  }

  return (
    <div className="space-y-6 w-full px-2 sm:px-4 bg-[#E5E5E5] min-h-screen py-6">
      <h1 className="text-2xl md:text-3xl font-bold text-center text-[#14213D]">Event Locations</h1>
  ...
  {filtered.map((l) => (
    ...
    >
      <CardHeader className="flex flex-row items-center justify-between p-0">
        <CardTitle className="text-2xl font-semibold truncate">{l.name}</CardTitle>
        <Badge className={statusColors[l.status] || statusColors.default}>{l.status}</Badge>
      </CardHeader>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        <Input placeholder="Search locations..." value={search} onChange={(e) => setSearch(e.target.value)} className="flex-1 rounded-lg w-full bg-white" />
        <Button onClick={() => openModal()} className="rounded-lg px-6 w-full sm:w-auto bg-[#14213D] text-white hover:opacity-90 transition">+ Add Location</Button>
      </div>
        ...
        <Table className="w-full bg-[#14213D] text-white">
          ...
          <TableBody>
            {filtered.length > 0 ? filtered.map((l) => (
              <TableRow key={l.id} className="hover:bg-[#1f2b4d] text-sm md:text-base border-b border-gray-700">
                ...
                <TableCell className="px-4 py-4 text-center">
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button variant="ghost" size="sm" className="rounded-full text-white hover:bg-white/20"><MoreHorizontal /></Button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent className="bg-white text-[#14213D]">
                      <DropdownMenuItem onClick={() => openModal(l)}>Edit</DropdownMenuItem>
                      <DropdownMenuItem onClick={() => handleDelete(l.id)} className="text-red-600 cursor-pointer">Delete</DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </TableCell>
              </TableRow>
            )) : (
              <TableRow><TableCell colSpan={6} className="text-center py-6 text-gray-300">No locations found.</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </div>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-lg w-[95%] rounded-xl">
          <DialogHeader>
            <DialogTitle className="text-lg font-semibold text-[#14213D]">{editingLocation ? "Edit Location" : "Add Location"}</DialogTitle>
          </DialogHeader>

          <div className="flex flex-col gap-4 mt-3">
            <Input placeholder="Name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="bg-gray-50" />
            <Input type="number" placeholder="Capacity" value={form.capacity} onChange={(e) => setForm({ ...form, capacity: parseInt(e.target.value) || 0 })} className="bg-gray-50" />
            <Input placeholder="Owner Name" value={form.owner} onChange={(e) => setForm({ ...form, owner: e.target.value })} className="bg-gray-50" />
            ...
            <Select value={form.status} onValueChange={(value) => setForm({ ...form, status: value })}>
              <SelectTrigger className="w-full bg-gray-50 rounded-lg p-2"><SelectValue placeholder="Select Status" /></SelectTrigger>
              <SelectContent>
          ...
          <DialogFooter className="mt-4">
            <Button onClick={handleSave} className="rounded-lg px-6 bg-gradient-to-r from-[#14213D] to-[#6B7280] text-white w-full sm:w-auto">{editingLocation ? "Update Location" : "Save Location"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
